...
{
	msonlab::Types::DataType t1 = (stack->top())->getValue();
	stack->pop();
	msonlab::Types::DataType t2 = (stack->top())->getValue();
	stack->pop();

	stack->push(StackValue::stackvaluePtr(new SimpleStackValue(Types::DataType(new double(*t1 * *t2)))));
}
...
void msonlab::StackRunner::power(dPtrStack* stack, StackValue::stackvaluePtr data, srPtr runner, int sync_id)
{
	...
	stack->push(StackValue::stackvaluePtr(new SimpleStackValue(Types::DataType(new double(pow(*t2, *t1))))));
}
...
void msonlab::StackRunner::sub(dPtrStack* stack, StackValue::stackvaluePtr data, srPtr runner, int sync_id)
{
	...
	stack->push(StackValue::stackvaluePtr(new SimpleStackValue(Types::DataType(new double(*t2 - *t1)))));
}
...
void msonlab::StackRunner::run(srPtr runner, int count)
{
	for (size_t i = 0; i < count; ++i)
	{
		...
		{
			auto prom = StackRunner::promisePtr(new std::promise<Types::DataType>());
			promises.push_back(std::pair<unsigned int, StackRunner::promisePtr>(i_id, prom));
			futures.push_back(std::pair<unsigned int, StackRunner::futurePtr>(i_id, StackRunner::futurePtr(new std::shared_future<Types::DataType>(prom->get_future()))));
		}
